...
---@param content string The raw content of the .env file.
...
---@param parsed_env table The parsed environment variables
---@param options table Options for enxport behavior
...
function lenv.export(parsed_env, options)
    ...
    if options.script_file then
        ...
        if not filename:match("%.[^.]*$") then
            filename = filename .. outputs.script_extention
        end

        local file, write_err = io.open(filename, "w")
        if file then
            file:write(outputs.scripts_content)
            file:close()
